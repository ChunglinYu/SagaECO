...
namespace SagaDB
{
    public class SQLiteActorDB : SQLiteConnectivity, ActorDB
    {
        ...
        public SQLiteActorDB(string Source) : base()
        {
            this.Source = Source;
        ...
        public void CreateChar(ActorPC aChar, int account_id)
        {
            ...
            if (aChar != null && this.isConnected() == true)
            {
                ...
                aChar.CharID = charID;
                SQLiteCommand cmd = new SQLiteCommand(string.Format("INSERT INTO `inventory`(`char_id`,`data`) VALUES ('{0}',@data);\r\n", aChar.CharID));
                cmd.Parameters.Add("@data", DbType.Binary).Value = aChar.Inventory.ToBytes();
                ...
                if (aChar.Inventory.WareHouse != null)
                {
                    ...
                    {
                        cmd = new SQLiteCommand(string.Format("INSERT INTO `warehouse`(`account_id`,`data`) VALUES ('{0}',@data);\r\n", account_id));
                        cmd.Parameters.Add("@data", DbType.Binary).Value = aChar.Inventory.WareToBytes();
        ...
        public ActorPartner GetActorPartner(uint ActorPartnerID, Item.Item partneritem)
        {
            ...
            if (result.Count > 0)
            {
                uint partnerid = Convert.ToUInt32(result[0]["pid"]);
                ...
                ap.reliabilityexp = Convert.ToUInt64(result[0]["texp"]);
                if (((byte)result[0]["rb"]) == 0)
                    ap.rebirth = false;
                else
                    ap.rebirth = true;
                ap.rank = (byte)result[0]["rank"];
                ap.perkpoint = Convert.ToUInt16(result[0]["perkspoints"]);
                ap.HP = Convert.ToUInt32(result[0]["hp"]);
                ap.MaxHP = Convert.ToUInt32(result[0]["maxhp"]);
                ap.MP = Convert.ToUInt32(result[0]["mp"]);
                ap.MaxMP = Convert.ToUInt32(result[0]["maxmp"]);
                ap.SP = Convert.ToUInt32(result[0]["sp"]);
                ap.MaxSP = Convert.ToUInt32(result[0]["maxsp"]);
                ...
                ap.exp = Convert.ToUInt64(result[0]["exp"]);
                ap.nextfeedtime = (DateTime)result[0]["nextfeedtime"];
                ap.reliabilityuprate = Convert.ToUInt16(result[0]["reliabilityuprate"]);
        ...
        public void GetPartnerEquip(ActorPartner ap)
        {
            ...
            if (result.Count > 0)
            {
                ...
                {
                    Item.Item item = SagaDB.Item.ItemFactory.Instance.GetItem(Convert.ToUInt32(i["item_id"]));
                    item.Stack = Convert.ToUInt16(i["count"]);
        ...
        public void GetPartnerCube(ActorPartner ap)
        {
            ...
            if (result.Count > 0)
            {
                ...
                {
                    if ((byte)i["type"] == 1)
                        ap.equipcubes_condition.Add(Convert.ToUInt16(i["unique_id"]));
                    if ((byte)i["type"] == 2)
                        ap.equipcubes_action.Add(Convert.ToUInt16(i["unique_id"]));
                    if ((byte)i["type"] == 3)
                        ap.equipcubes_activeskill.Add(Convert.ToUInt16(i["unique_id"]));
                    if ((byte)i["type"] == 4)
                        ap.equipcubes_passiveskill.Add(Convert.ToUInt16(i["unique_id"]));
                }
            }
        }
        public void GetPartnerAI(ActorPartner ap)
        {
            ...
            if (result.Count > 0)
            {
                foreach (DataRow i in result)
                {
                    if ((byte)i["type"] == 1)
                    {
                        ap.ai_conditions.Add((byte)i["index"], Convert.ToUInt16(i["value"]));
                    }
                    if ((byte)i["type"] == 2)
                    {
                        ap.ai_reactions.Add((byte)i["index"], Convert.ToUInt16(i["value"]));
                    }
                    if ((byte)i["type"] == 3)
                    {
                        ap.ai_intervals.Add((byte)i["index"], Convert.ToUInt16(i["value"]));
                    }
                    if ((byte)i["type"] == 4)
                    {
                        ap.ai_states.Add((byte)i["index"], Convert.ToBoolean(Convert.ToUInt16(i["value"])));
                    }
                }
            }
        }
        private uint getCharID(string name)
        {
            ...
            return Convert.ToUInt32(result[0]["charID"]);
                ...
                try
                {
                    ...
                    if (result.Count > 0)
                    {
                        for (int i = 0; i < result.Count; i++)
                        {
                            ...
                            uint mobid = Convert.ToUInt32(result[i]["object_id"]);
                            uint c = Convert.ToUInt32(result[i]["count"]);
                            ki.Count = (int)c;
                            c = Convert.ToUInt32(result[i]["totalcount"]);
                            ki.TotalCount = (int)c;
        ...
        */
        ...
        {
            string sqlstr;
            uint account_id = Convert.ToUInt32(SQLExecuteQuery("SELECT `account_id` FROM `char` WHERE `char_id`='" + aChar.CharID + "' LIMIT 1")[0]["account_id"]);
        ...
        public ActorPC GetChar(uint charID, bool fullinfo)
        {
            ...
            try
            {
                ...
                pc.UsingPaperID = Convert.ToUInt16(result["usingpaper_id"]);
                pc.PlayerTitleID = Convert.ToUInt16(result["title_id"]);
                ...
                pc.HairStyle = Convert.ToUInt16(result["hairStyle"]);
                pc.HairColor = (byte)result["hairColor"];
                pc.Wig = Convert.ToUInt16(result["wig"]);
                pc.Face = Convert.ToUInt16(result["face"]);
                pc.Job = (PC_JOB)(byte)result["job"];
                pc.MapID = Convert.ToUInt32(result["mapID"]);
                ...
                pc.QuestRemaining = Convert.ToUInt16(result["questRemaining"]);
                pc.QuestNextResetTime = (DateTime)result["questresettime"];
                pc.Fame = Convert.ToUInt32(result["fame"]);
                ...
                pc.SaveMap = Convert.ToUInt32(result["save_map"]);
                pc.SaveX = (byte)result["save_x"];
                pc.SaveY = (byte)result["save_y"];
                pc.HP = Convert.ToUInt32(result["hp"]);
                pc.MP = Convert.ToUInt32(result["mp"]);
                pc.SP = Convert.ToUInt32(result["sp"]);
                pc.MaxHP = Convert.ToUInt32(result["max_hp"]);
                pc.MaxMP = Convert.ToUInt32(result["max_mp"]);
                pc.MaxSP = Convert.ToUInt32(result["max_sp"]);
                pc.EP = Convert.ToUInt32(result["ep"]);
                pc.EPLoginTime = (DateTime)result["eplogindate"];
                pc.EPGreetingTime = (DateTime)result["epgreetingdate"];
                pc.EPUsed = Convert.ToInt16(result["epused"]);
                pc.CL = Convert.ToInt16(result["cl"]);
                pc.Str = Convert.ToUInt16(result["str"]);
                pc.Dex = Convert.ToUInt16(result["dex"]);
                pc.Int = Convert.ToUInt16(result["intel"]);
                pc.Vit = Convert.ToUInt16(result["vit"]);
                pc.Agi = Convert.ToUInt16(result["agi"]);
                pc.Mag = Convert.ToUInt16(result["mag"]);
                pc.StatsPoint = Convert.ToUInt16(result["statspoint"]);
                pc.SkillPoint = Convert.ToUInt16(result["skillpoint"]);
                pc.SkillPoint2X = Convert.ToUInt16(result["skillpoint2x"]);
                pc.SkillPoint2T = Convert.ToUInt16(result["skillpoint2t"]);
                pc.SkillPoint3 = Convert.ToUInt16(result["skillpoint3"]);
                pc.EXStatPoint = Convert.ToUInt16(result["exstatpoint"]);
                ...
                lock (this)
                {
                    ...
                    pc.Gold = Convert.ToInt64(result["gold"]);
                    Logger.SQLLogLevel.Value = old;
                }
                pc.CP = Convert.ToUInt32(result["cp"]);
                pc.ECoin = Convert.ToUInt32(result["ecoin"]);
                pc.CEXP = Convert.ToUInt64(result["cexp"]);
                //pc.JEXP = (ulong)result["jexp"];
                pc.JointJEXP = Convert.ToUInt64(result["jjexp"]);
                pc.ExplorerEXP = Convert.ToUInt64(result["explorerEXP"]);
                pc.WRP = Convert.ToInt32(result["wrp"]);
                pc.PossessionTarget = Convert.ToUInt32(result["possession_target"]);
                pc.AbyssFloor = Convert.ToInt32(result["abyssfloor"]);
                Party.Party party = new SagaDB.Party.Party();
                party.ID = Convert.ToUInt32(result["party"]);
                Ring.Ring ring = new SagaDB.Ring.Ring();
                ring.ID = Convert.ToUInt32(result["ring"]);
                if (party.ID != 0)
                    pc.Party = party;
                if (ring.ID != 0)
                    pc.Ring = ring;
                uint golem = Convert.ToUInt32(result["golem"]);
                if (golem != 0)
                {
                ...
                if (fullinfo)
                {
                    uint questid = Convert.ToUInt32(result["questid"]);
                    if (questid != 0)
                    {
                        try
                        {
                            ...
                            quest.CurrentCount1 = Convert.ToInt32(result["questcurrentcount1"]);
                            quest.CurrentCount2 = Convert.ToInt32(result["questcurrentcount2"]);
                            quest.CurrentCount3 = Convert.ToInt32(result["questcurrentcount3"]);
                            pc.Quest = quest;
                        }
                        catch { }
                    }
        ...
        public void GetVShop(ActorPC pc)
        {
            ...
            pc.e = null;
            pc.VShopPoints = Convert.ToUInt32(result["vshop_points"]);
            pc.e = eh;
            pc.UsedVShopPoints = Convert.ToUInt32(result["used_vshop_points"]);
        }
        public void SaveSkill(ActorPC pc)
        {
            ...
            SQLiteCommand cmd = new SQLiteCommand(string.Format("REPLACE INTO `skill`(`char_id`,`skills`,`jobbasic`,`joblv`,`jobexp`,`skillpoint`,`skillpoint2x`," +
            ...
            cmd.Parameters.Add("@data", DbType.Binary).Value = ms.ToArray();
            ms.Close();
            try
            {
                //SQLExecuteNonQuery(sqlstr);
        ...
        public void SaveServerVar(ActorPC fakepc)
        {
            ...
            string sqlstr = "DELETE FROM `sVar`;";
            ...
            sqlstr = "DELETE FROM `sList`;";
            foreach (var item in fakepc.Adict)
            {
                foreach (var i in item.Value.Keys)
                {
                    sqlstr += string.Format("INSERT INTO `sList`(`ServerVarID`,`name`,`key`,`type`,`content`) VALUES " + "('{0}','{1}','{2}',1,'{3}');", Guid.NewGuid(), item.Key, i, item.Value[i]);
                }
            }
            SQLExecuteNonQuery(sqlstr);
        }
        ...
        {
            uint account_id = Convert.ToUInt32(SQLExecuteQuery("SELECT `account_id` FROM `char` WHERE `char_id`='" + pc.CharID + "' LIMIT 1")[0]["account_id"]);
            ...
            //SQLiteCommand cmd = new SQLiteCommand(string.Format("REPLACE `cvar`(`char_id`,`values`) VALUES ('{0}',@data);",pc.CharID));
            SQLiteCommand cmd = new SQLiteCommand(string.Format("delete from cvar where char_id={0};insert into cvar values({0},@data);", pc.CharID));
            ...
            //cmd = new SQLiteCommand(string.Format("REPLACE INTO `avar`(`account_id`,`values`) VALUES ('{0}',@data); ",account_id));
            cmd = new SQLiteCommand(string.Format("delete from avar where account_id={0};insert into avar values({0},@data);", account_id));
            cmd.Parameters.Add("@data", DbType.Binary).Value = ms.ToArray();
            ms.Close();
            try
            {
        ...
        public void SaveItem(ActorPC pc)
        {
            try
            {
                ...
                {
                    cmd = new SQLiteCommand(string.Format("UPDATE `inventory` SET `data`=@data WHERE `char_id`='{0}' LIMIT 1;\r\n",
                        pc.CharID));
                    byte[] itemdata = pc.Inventory.ToBytes();
                    cmd.Parameters.Add("@data", DbType.Binary).Value = itemdata;

                    if (pc.Account != null)
                        Logger.ShowInfo("儲存玩家(" + pc.Account.AccountID.ToString() + ")：" + pc.Name + "道具信息...大小：" + itemdata.Length);
                    try
                    {
                        SQLExecuteNonQuery(cmd);
                    }
                ...
                if (pc.Inventory.WareHouse != null)
                {
                    ...
                    {
                        cmd = new SQLiteCommand(string.Format("UPDATE `warehouse` SET `data`=@data WHERE `account_id`='{0}' LIMIT 1;\r\n",
                        account));
                        SQLiteParameter para = cmd.Parameters.Add("@data", DbType.Binary);
        ...
        public void GetSkill(ActorPC pc)
        {
            try
            {
                ...
                if (result.Count > 0)
                {
                    ...
                    pc.JEXP = Convert.ToUInt64( result[0]["jobexp"]);
                    pc.SkillPoint = Convert.ToUInt16(result[0]["skillpoint"]);
                    pc.SkillPoint2X = Convert.ToUInt16(result[0]["skillpoint2x"]);
                    pc.SkillPoint2T = Convert.ToUInt16(result[0]["skillpoint2t"]);
                    pc.SkillPoint3 = Convert.ToUInt16(result[0]["skillpoint3"]);
        ...
        public void GetNPCStates(ActorPC pc)
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                uint npcID = Convert.ToUInt32(result[i]["npc_id"]);
        ...
        public uint GetAccountID(uint charID)
        {
            ...
            if (result.Count == 0)
                ...
                return Convert.ToUInt32(result[0]["account_id"]);
        }
        ...
        public uint[] GetCharIDs(int account_id)
        {
            ...
            {
                buf[i] = Convert.ToUInt32(result[i]["char_id"]);
            }
            return buf;
        }
        ...
        public List<ActorPC> GetFriendList(ActorPC pc)
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                uint friend = Convert.ToUInt32(result[i]["friend_char_id"]);
        ...
        public List<ActorPC> GetFriendList2(ActorPC pc)
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                uint friend = Convert.ToUInt32(result[i]["char_id"]);
                ActorPC chara = new ActorPC();
                chara.CharID = friend;
        ...
        public Party.Party GetParty(uint id)
        {
            ...
            {
                party.ID = id;
                uint leader = Convert.ToUInt32(result[0]["leader"]);
        ...
        public Ring.Ring GetRing(uint id)
        {
            ...
            if (result.Count != 0)
            {
                ...
                ring.Fame = Convert.ToUInt32(result[0]["fame"]);
                if (result[0]["ff_id"] != null)
                    ring.FF_ID = Convert.ToUInt32(result[0]["ff_id"]);
                sqlstr = "SELECT * FROM `ringmember` WHERE `ring_id`='" + id + "';";
                DataRowCollection result2 = SQLExecuteQuery(sqlstr);
                for (int i = 0; i < result2.Count; i++)
                {
                    ActorPC pc = new ActorPC();
                    pc.CharID = Convert.ToUInt32(result2[i]["char_id"]);
                    ...
                    if (rows.Count > 0)
                    {
                        ...
                        if (index >= 0)
                            ring.Rights[index].Value = Convert.ToInt32(result2[i]["right"]);
                        if (leader == pc.CharID)
                            ring.Leader = pc;
                    }
                }
        ...
        public List<BBS.Mail> GetMail(ActorPC pc)
        {
            ...
            foreach (DataRow i in result)
            {
                ...
                post.MailID = Convert.ToUInt32(i["mail_id"]);
        ...
        public List<BBS.Gift> GetGifts(ActorPC pc)
        {
            ...
            foreach (DataRow i in result)
            {
                ...
                post.MailID = Convert.ToUInt32(i["mail_id"]);
                post.AccountID = Convert.ToUInt32(i["a_id"]);
                post.Name = (string)i["sender"];
                post.Title = (string)i["title"];
                post.Date = (DateTime)i["postdate"];
                ...
                for (int y = 1; y < 11; y++)
                {
                    uint itemid = Convert.ToUInt32((i["itemid" + y.ToString()]));
                    ushort count = Convert.ToUInt16((i["count" + y.ToString()]));
        ...
        private void GetFGarden(ActorPC pc)
        {
            ...
            if (result.Count > 0)
            {
                ...
                garden.ID = Convert.ToUInt32(result[0]["fgarden_id"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.FLYING_BASE] = Convert.ToUInt32(result[0]["part1"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.FLYING_SAIL] = Convert.ToUInt32(result[0]["part2"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.GARDEN_FLOOR] = Convert.ToUInt32(result[0]["part3"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.GARDEN_MODELHOUSE] = Convert.ToUInt32(result[0]["part4"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.HouseOutSideWall] = Convert.ToUInt32(result[0]["part5"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.HouseRoof] = Convert.ToUInt32(result[0]["part6"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.ROOM_FLOOR] = Convert.ToUInt32(result[0]["part7"]);
                garden.FGardenEquipments[SagaDB.FGarden.FGardenSlot.ROOM_WALL] = Convert.ToUInt32(result[0]["part8"]);
                garden.Fuel = Convert.ToUInt32(result[0]["fuel"]);
                pc.FGarden = garden;
            }
            if (pc.FGarden == null)
                return;
            ...
            foreach (DataRow i in result)
            {
                ...
                actor.ItemID = Convert.ToUInt32(i["item_id"]);
                actor.PictID = Convert.ToUInt32(i["pict_id"]);
                actor.X = Convert.ToInt16(i["x"]);
                actor.Y = Convert.ToInt16(i["y"]);
                actor.Z = Convert.ToInt16(i["z"]);
                actor.Xaxis = Convert.ToInt16(i["xaxis"]);
                actor.Yaxis = Convert.ToInt16(i["yaxis"]);
                actor.Zaxis = Convert.ToInt16(i["zaxis"]);
                //actor.Dir = (ushort)i["dir"];
                actor.Motion = Convert.ToUInt16(i["motion"]);
        ...
        public uint GetFFRindID(uint ffid)
        {
            ...
            if (result.Count > 0)
            {
                return Convert.ToUInt32(result[0]["ring_id"]);
            }
            else
                return 0;
        }
            ...
            if (pc.Ring != null)
            {
                ...
                if (result.Count > 0)
                {
                    if (pc.Ring.FFarden == null)
                    {
                        ...
                        pc.Ring.FFarden.ID = Convert.ToUInt32(result[0]["ff_id"]);
                        pc.Ring.FFarden.Name = (string)result[0]["name"];
                        pc.Ring.FFarden.RingID = Convert.ToUInt32(result[0]["ring_id"]);
                        pc.Ring.FFarden.ObMode = 3;
                        pc.Ring.FFarden.Content = (string)result[0]["content"];
                        pc.Ring.FFarden.Level = Convert.ToUInt32(result[0]["level"]);
                    }
                }
            }
        }
        ...
        public List<SagaDB.FFarden.FFarden> GetFFList()
        {
            ...
            foreach (DataRow i in result)
            {
                ...
                ff.ID = Convert.ToUInt32(i["ff_id"]);
                ff.RingID = Convert.ToUInt32(i["ring_id"]);
                ff.Level = Convert.ToUInt32(i["level"]);
                list.Add(ff);
            }
        ...
        public void GetPaper(ActorPC pc)
        {
            ...
            foreach (DataRow i in result)
            {
                uint paperid = Convert.ToUInt32(i["paper_id"]);
                AnotherDetail detail = new AnotherDetail();
                detail.value = new BitMask_Long();
                detail.value.Value = Convert.ToUInt64(i["paper_value"]);
    ...
    pc.Ring.FF_ID, pc.Ring.ID);
        ...
        public List<ActorPC> GetWRPRanking()
        {
            ...
            {
                ActorPC pc = new ActorPC();
                pc.CharID = Convert.ToUInt32(i["char_id"]);
        ...
        public void GetLevelLimit()
        {
            ...
            {
                levellimit.NowLevelLimit = Convert.ToUInt32(i["NowLevelLimit"]);
                levellimit.NextLevelLimit = Convert.ToUInt32(i["NextLevelLimit"]);
                levellimit.SetNextUpLevelLimit = Convert.ToUInt32(i["SetNextUpLevel"]);
                levellimit.LastTimeLevelLimit = Convert.ToUInt32(i["LastTimeLevelLimit"]);
                levellimit.SetNextUpDays = Convert.ToUInt32(i["SetNextUpDays"]);
                levellimit.ReachTime = (DateTime)i["ReachTime"];
                levellimit.NextTime = (DateTime)i["NextTime"];
                levellimit.FirstPlayer = Convert.ToUInt32(i["FirstPlayer"]);
                levellimit.SecondPlayer = Convert.ToUInt32(i["SecondPlayer"]);
                levellimit.Thirdlayer = Convert.ToUInt32(i["ThirdPlayer"]);
                levellimit.FourthPlayer = Convert.ToUInt32(i["FourthPlayer"]);
                levellimit.FifthPlayer = Convert.ToUInt32(i["FifthPlayer"]);
        ...
        public List<Tamaire.TamaireLending> GetTamaireLendings()
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                ...
                tamaireLending.Lender = Convert.ToUInt32(result[0]["char_id"]);
                ...
                for (byte j = 1; j <= 4; j++)
                {
                    uint renterid = Convert.ToUInt32(result[0]["renter" + j.ToString()]);
                    if (renterid != 0)
        ...
        public void GetTamaireLending(ActorPC pc)
        {
            ...
            if (result.Count != 0)
            {
                ...
                pc.TamaireLending.Lender = Convert.ToUInt32(result[0]["char_id"]);
        ...
        public void GetTamaireRental(ActorPC pc)
        {
            ...
            if (result.Count != 0)
            {
                ...
                pc.TamaireRental.Renter = Convert.ToUInt32(result[0]["char_id"]);
                pc.TamaireRental.RentDue = (DateTime)result[0]["rentdue"];
                pc.TamaireRental.CurrentLender = Convert.ToUInt32(result[0]["currentlender"]);
                pc.TamaireRental.LastLender = Convert.ToUInt32(result[0]["lastlender"]);
            }
        }
        ...
        public void SaveDualJobInfo(ActorPC pc, bool allinfo)
        {
            ...
            {
                insertstr += $"insert into `dualjob` values ({Guid.NewGuid()},{pc.CharID}, {dic[item].DualJobID},{dic[item].DualJobLevel}, {dic[item].DualJobExp});";
            }
            ...
            if (allinfo)
            {
                ...
                {
                    delskillstr += $"insert into `dualjob_skill` values ({Guid.NewGuid()},{pc.CharID}, {pc.DualJobID}, {item.ID}, {item.Level});";
                }
